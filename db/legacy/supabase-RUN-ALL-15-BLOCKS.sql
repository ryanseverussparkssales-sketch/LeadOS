-- ============================================================
-- LeadOS — ALL 15 PENDING MIGRATIONS
-- Run each block SEPARATELY in Supabase SQL Editor.
-- Copy one block at a time, paste, run, confirm success,
-- then move to the next. All use IF NOT EXISTS — safe to re-run.
-- ============================================================
...
-- ┌─────────────────────────────────────────────────────────┐
-- │  BLOCK 3: Spotify tokens + Widget settings + Time entries│
-- └─────────────────────────────────────────────────────────┘
...
-- ┌─────────────────────────────────────────────────────────┐
-- │  BLOCK 4: Contact Activities table                      │
-- └─────────────────────────────────────────────────────────┘
...
-- │  Safe if clients don't exist yet — skips silently       │
-- └─────────────────────────────────────────────────────────┘
DO $$
DECLARE
...
BEGIN
  IF v_sparks_id IS NOT NULL THEN
    ...
    (v_user_id, v_sparks_id, 'Talking Points',
     'You get 10+ years of cold calling experience without the cost of a full-time SDR. We''ve built over $2M in pipeline through outbound alone. Results-focused: appointments set and pipeline created.',
     'talking_points', 3)
  ...
  IF v_welfel_id IS NOT NULL THEN
    ...
    (v_user_id, v_welfel_id, 'Company Overview',
     'Welfel Ventures is a DTC brand portfolio company founded 2017 by Bryan and Peter Welfel. Portfolio: The Beard Club ($22.4M raised), iotty Smart Home (WiFi smart switches, $10M+ projected 2026), Huntt.gg (gaming rewards, launched 2024).',
     'general', 1),
    (v_user_id, v_welfel_id, 'Key Contacts',
     'Bryan Welfel — Co-Founder, CEO The Beard Club. Twitter: @bwelfel. New York, NY. Founded JSwipe (sold 2015). Peter Welfel — Co-Founder, Operations.',
     'general', 2),
    (v_user_id, v_welfel_id, 'Outreach Strategy',
     'iotty: Target electricians (B2B high LTV), home builders, interior designers. Huntt.gg: Target gaming streamers, esports teams, Gen Z brands. Beard Club: Men''s lifestyle, subscription retailers. Ryan role: CS for iotty + streamer outreach for Huntt.',
     'talking_points', 3)
    ON CONFLICT DO NOTHING;
  END IF;
END $$;
...
-- ┌─────────────────────────────────────────────────────────┐
-- │  BLOCK 12: Wrench Node + Huntt projects setup           │
-- └─────────────────────────────────────────────────────────┘
DO $$
DECLARE
  v_user_id UUID := (SELECT id FROM auth.users LIMIT 1);
  ...
  IF v_wrench_id IS NOT NULL THEN
    ...
    IF v_wrench_proj_id IS NOT NULL THEN
      ...
      VALUES (v_user_id, v_wrench_id, 'Project Details',
        '70 contacts verified (not 300). $175 of $350 paid, $175 outstanding. 6hrs calling remaining. Reprice next project to $50-75/hr. Deliverable: booked appointments or qualified leads.',
        'general')
      ON CONFLICT DO NOTHING;
    END IF;
  END IF;
...
BEGIN
  ...
  VALUES
    ...
    (v_user_id, 'Wrench Node power calling session', 'pending', 'urgent', 'follow_up', '6 hrs left. $175 still outstanding. Score warm leads first.'),
    (v_user_id, 'Havens power calling session', 'pending', 'urgent', 'follow_up', '8-11am block. $100/meeting set.'),
    ...
    (v_user_id, 'Email forwarding setup', 'pending', 'high', 'task', 'Forward all Gmails + Outlook to one main Gmail.'),
    (v_user_id, 'Bill Welfel the $44 tool cost', 'pending', 'high', 'follow_up', 'Add to next Bryan invoice.'),
    (v_user_id, 'Refine Streamer Outreach Proof', 'pending', 'high', 'follow_up', 'Refine the Huntt.gg streamer outreach proposal.'),
    (v_user_id, 'Insurance coverage verification', 'pending', 'medium', 'task', 'Re-verify coverage before appointment.'),
    (v_user_id, 'Inbox nuclear option', 'pending', 'medium', 'task', 'Gmail: search older_than:6m -> select all -> archive. All 3 Gmails.'),
    (v_user_id, 'Pass tool costs to clients', 'pending', 'medium', 'task', 'Adobe $38 + Canva $14 + tool costs = ~$74/mo billable back to clients.'),
    (v_user_id, 'Find ghost subscriptions', 'pending', 'medium', 'task', 'Open bank app, filter recurring charges. Target: $100 random savings.'),
    (v_user_id, 'Sparks Curiosity LLC formation', 'pending', 'medium', 'task', 'Critical for mortgage qualification.'),
    ...
    (v_user_id, 'Build Sparks Command Center - Phase 1', 'pending', 'medium', 'task', 'Full LeadOS setup for Sparks Curiosity Studio.'),
    (v_user_id, 'Kill CC debt $1,200', 'pending', 'high', 'task', 'Use Welfel payment + roommate funds.'),
    (v_user_id, 'NAS setup', 'pending', 'low', 'task', '2-bay starter NAS + drives $400-600.'),
    (v_user_id, 'Steam Deck setup', 'pending', 'low', 'task', '9pm-12am gaming block. Protect decompression time.'),
    (v_user_id, 'Home automation + mini PC', 'pending', 'low', 'task', 'Always-on mini PC for NAS + home automation.'),
    (v_user_id, 'Desk rework + cable management', 'pending', 'low', 'task', '90in IKEA top. 5-screen layout.')
  ON CONFLICT DO NOTHING;
END $$;


-- ┌─────────────────────────────────────────────────────────┐
-- │  BLOCK 14: Campaign types                               │
-- └─────────────────────────────────────────────────────────┘
...
-- ┌─────────────────────────────────────────────────────────┐
-- │  BLOCK 15: Client profile fields                        │
-- └─────────────────────────────────────────────────────────┘
