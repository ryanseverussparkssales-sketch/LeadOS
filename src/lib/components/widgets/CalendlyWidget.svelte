...
<div class="flex flex-col h-full">
    {#if editing || !calendlyUrl}
        <div class="flex flex-col justify-center items-center h-full p-4 gap-3">
            ...
            <p class="text-xs text-[#7c7c7c] text-center">Enter your Calendly URL to embed your scheduling page</p>
            ...
            <div class="flex gap-2 w-full">
                ...
                {#if calendlyUrl}
                    <button onclick={() => editing = false} class="flex-1 rounded-lg border border-[#2a2a2a] py-2 text-xs text-[#8a8a8a]">Cancel</button>
                {/if}
            </div>
        </div>
    {:else}
        <div class="flex items-center justify-between px-3 py-2 border-b border-[#1a1a1a] flex-shrink-0">
            ...
            <button onclick={() => editing = true} class="text-[10px] text-[#6e6e6e] hover:text-white">Edit</button>
        </div>
        <iframe
