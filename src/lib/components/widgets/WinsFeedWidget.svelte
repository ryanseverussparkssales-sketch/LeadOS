...
<div class="flex flex-col h-full p-3">
    ...
    {:else}
        <div class="flex-1 overflow-y-auto space-y-1">
            {#each wins as win}
                <div class="rounded-lg bg-green-950/20 border border-green-900/30 px-3 py-2 flex items-center justify-between">
                    <div>
                        <p class="text-xs text-green-400 font-medium">{win.contact?.name ?? 'Unknown'}</p>
                        <p class="text-[10px] text-[#555]">{win.outcome?.replace(/_/g,' ')} · {win.campaign?.name ?? 'Campaign'}</p>
                    </div>
                    <div class="flex items-center gap-2">
                        {#if win.weight > 1}<span class="text-[10px] text-green-400 font-mono">×{win.weight}</span>{/if}
