            ...
            {#each campaigns as c}
                <div>
                    ...
                    <div class="w-full h-1.5 bg-[#1a1a1a] rounded-full overflow-hidden">
                        <div class="h-full rounded-full transition-all {pct(c.calls_today, c.daily_call_goal) >= 100 ? 'bg-green-500' : pct(c.calls_today, c.daily_call_goal) >= 60 ? 'bg-yellow-500' : 'bg-white/40'}"
                    ...
                    {#if c.target_wins}
                        <div class="flex justify-between text-[10px] mt-1.5 mb-0.5">
                            ...
                            <span class="text-green-400 font-mono">{c.win_count}/{c.target_wins}</span>
                        </div>
                        <div class="w-full h-1 bg-[#1a1a1a] rounded-full overflow-hidden">
                            <div class="h-full bg-green-500 rounded-full" style="width:{pct(c.win_count, c.target_wins)}%"></div>
                        </div>
                    {/if}
                </div>
            {/each}
