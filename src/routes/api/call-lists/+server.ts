...
export const GET: RequestHandler = async ({ request, url }) => {
	...
	let query = supabaseAdmin
		.from('call_lists')
		.select('*, campaign:campaigns(id, name, project:projects(id, name, client:clients(id, name, user_id))), call_list_contacts(count)')
		.order('name');
	...
	const filtered = (data ?? []).filter(l => {
		const uid = (l.campaign as unknown as { project?: { client?: { user_id: string } } })?.project?.client?.user_id;
		return uid === user.id;
	});
...
export const POST: RequestHandler = async ({ request }) => {
	...
	const { data, error: e } = await supabaseAdmin
		.from('call_lists')
		.insert({ project_id: directProjectId, name: name.trim(), status: 'active' })
		.select()
		.single();
	if (e) throw error(400, e.message);
	return json(data);
