...
${(d.routingNumber || d.accountNumber) ? `<div style="margin-top:40px;padding-top:20px;border-top:1px solid #eee"><div style="font-size:12px;text-transform:uppercase;letter-spacing:1px;color:#666;margin-bottom:12px">ACH Payment Info</div><table style="width:100%;font-size:14px;border-collapse:collapse">${d.bankName ? '<tr><td style="color:#666;width:40%">Bank</td><td>' + d.bankName + '</td></tr>' : ''}${d.routingNumber ? '<tr><td style="color:#666">Routing #</td><td style="font-family:monospace">' + d.routingNumber + '</td></tr>' : ''}${d.accountNumber ? '<tr><td style="color:#666">Account #</td><td style="font-family:monospace">' + d.accountNumber + '</td></tr>' : ''}${d.paymentMemo ? '<tr><td style="color:#666">Memo</td><td>' + d.paymentMemo + '</td></tr>' : ''}</table></div>` : ''}<p style="margin-top:60px;font-size:12px;color:#666">Generated by RogueOS · ${new Date().toLocaleDateString()}</p>
</body></html>`;
}
