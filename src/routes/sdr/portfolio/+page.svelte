...
<div class="flex-1 overflow-y-auto p-8 max-w-3xl mx-auto space-y-8">
	...
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="text-xs text-[#7c7c7c] mt-0.5">Your best call highlights and testimonials, kept in one place</p>
		</div>
	...
	<div>
		<div class="flex items-center justify-between mb-3">
			<p class="text-xs text-[#7c7c7c] uppercase tracking-widest">📌 Featured on profile ({featuredClips.length}/3 max)</p>
		...
		{#if featuredClips.length === 0}
			<div class="rounded-xl border border-dashed border-[#2a2a2a] p-6 text-center">
				<p class="text-xs text-[#6e6e6e]">No featured clips yet — star clips below to pin them to your profile</p>
			</div>
		{:else}
			<div class="space-y-2">
				{#each featuredClips as clip}
					<div class="rounded-xl border border-yellow-800/30 bg-yellow-950/10 p-4 flex items-start gap-3">
						...
						<div class="flex-1 min-w-0">
							<p class="text-xs text-[#8a8a8a] uppercase tracking-widest mb-1">{CLIP_LABELS[clip.clip_type]}</p>
							{#if clip.transcript_excerpt}
								<p class="text-xs text-[#ccc] leading-relaxed italic">"{clip.transcript_excerpt.slice(0, 120)}…"</p>
							{/if}
							{#if clip.ai_reason}<p class="text-[9px] text-[#6e6e6e] mt-1">✦ {clip.ai_reason}</p>{/if}
						</div>
	...
	{:else if clips.length === 0}
		<div class="rounded-xl border border-[#2a2a2a] bg-[#111] p-8 text-center hover:border-[#262626] hover:bg-[#0f0f0f] transition-colors">
			...
			<p class="text-xs text-[#7c7c7c] mt-2">Make recorded calls — the AI will scan transcripts and pull your best moments.</p>
			<button onclick={generateSupercut} disabled={generating}
	...
	{:else if allClips.length > 0}
		<div>
			<p class="text-xs text-[#7c7c7c] uppercase tracking-widest mb-3">All clips ({allClips.length}) — star to feature</p>
			<div class="space-y-2">
	...
	<div>
		<div class="flex items-center justify-between mb-3">
			<p class="text-xs text-[#7c7c7c] uppercase tracking-widest">⭐ Testimonials ({testimonials.length})</p>
			<button onclick={() => showTestiForm = !showTestiForm}
				class="text-xs border border-[#2a2a2a] px-3 py-1 rounded-lg text-[#7c7c7c] hover:border-white hover:text-white transition-colors">
		...
		{#if showTestiForm}
			<div class="rounded-xl border border-[#2a2a2a] bg-[#111] p-5 mb-4 space-y-3 hover:border-[#262626] hover:bg-[#0f0f0f] transition-colors">
				<p class="text-xs text-[#7c7c7c]">Add a testimonial from a client or manager for your portfolio.</p>
		...
		{#if testimonials.length === 0 && !showTestiForm}
			<div class="rounded-xl border border-[#1a1a1a] bg-[#111] p-5 text-center">
				<p class="text-xs text-[#6e6e6e]">No testimonials yet. Ask a client or manager to write one — then add it here.</p>
			</div>
		{:else}
			<div class="space-y-3">
					...
					<div class="rounded-xl border border-[#2a2a2a] bg-[#111] p-4 group hover:border-[#262626] hover:bg-[#0f0f0f] transition-colors">
						<div class="flex items-start justify-between gap-3">
							<div class="flex-1">
								...
								<p class="text-xs text-[#7c7c7c] mt-2">— {t.client_name}{t.client_title ? `, ${t.client_title}` : ''}{t.client_company ? ` @ ${t.client_company}` : ''}</p>
							</div>
							<div class="flex items-center gap-2 shrink-0">
