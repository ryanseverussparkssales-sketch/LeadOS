<script lang="ts">
	...
	async function toggleFeatured(clip: Clip) {
		...
		await apiFetch(`/api/rep-profile/supercut?id=${clip.id}`, {
			method: 'PATCH',
	...
	async function removeClip(id: string) {
		// Use DELETE endpoint
		await apiFetch(`/api/rep-profile/supercut?id=${id}`, { method: 'DELETE' });
	...
	async function deleteTestimonial(id: string) {
		await apiFetch(`/api/rep-profile/testimonials?id=${id}`, { method: 'DELETE' });
...
<svelte:head><title>My Portfolio — LeadOS SDR</title></svelte:head>
...
<div class="flex-1 overflow-y-auto p-8 max-w-3xl mx-auto space-y-8">
	...
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="text-xs text-[#555] mt-0.5">Call highlights and testimonials shown on your public marketplace profile</p>
		</div>
		<button onclick={generateSupercut} disabled={generating}
			class="rounded-lg border border-purple-800/40 px-4 py-2 text-xs text-purple-400 hover:bg-purple-900/20 disabled:opacity-40 transition-colors">
	...
	<div>
		<div class="flex items-center justify-between mb-3">
			...
			<p class="text-[10px] text-[#333]">Only these show on your public profile</p>
		</div>
	...
	{:else if clips.length === 0}
		<div class="rounded-xl border border-[#2a2a2a] bg-[#111] p-8 text-center hover:border-[#262626] hover:bg-[#0f0f0f] transition-colors">
			...
			<button onclick={generateSupercut} disabled={generating}
				class="mt-4 rounded-lg border border-purple-800/40 px-4 py-2 text-xs text-purple-400 hover:bg-purple-900/20 disabled:opacity-40">
					...
					<div class="rounded-xl border border-[#2a2a2a] bg-[#111] p-4 flex items-start gap-3 group hover:border-[#262626] hover:bg-[#0f0f0f] transition-colors">
						...
						{#if clip.recording_url}
							<a href={clip.recording_url} target="_blank" rel="noopener noreferrer"
								class="text-[9px] text-purple-400 hover:text-purple-300 transition-colors mt-1 block">🎧 Listen</a>
						{/if}
						...
						<div class="flex items-center gap-2 shrink-0">
							...
							<button onclick={() => removeClip(clip.id)} class="text-[#222] hover:text-red-400 text-xs opacity-0 group-hover:opacity-100 transition-all">✕</button>
						</div>
					</div>
				{/each}
			</div>
	...
	<div>
		...
		{#if showTestiForm}
			<div class="rounded-xl border border-[#2a2a2a] bg-[#111] p-5 mb-4 space-y-3 hover:border-[#262626] hover:bg-[#0f0f0f] transition-colors">
				<p class="text-xs text-[#555]">Add a testimonial from a client or manager. It will appear on your public profile.</p>
